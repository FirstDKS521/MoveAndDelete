...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.imageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(self).with.insets(UIEdgeInsetsMake(0, 10, 20, 10));
        }];
...
- (void)setInEditState:(BOOL)inEditState
{
    if (inEditState && _inEditState != inEditState) {
        ...
        self.layer.borderColor = [UIColor sy_grayColor].CGColor;
...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = [UIColor sy_backColor];
